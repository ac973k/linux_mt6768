...
static void ovt_tcm_dispatch_report(struct ovt_tcm_hcd *tcm_hcd)
{
	...
	if (tcm_hcd->report.id == REPORT_TOUCH) {
		...
		if ((tcm_hcd->report.id == REPORT_IDENTIFY) && IS_FW_MODE(tcm_hcd->id_info.mode)) {
...
retry:
	...
	if (tcm_hcd->status_report_code <= STATUS_ERROR ||
		...
		case STATUS_CONTINUED_READ:
			LOGD(tcm_hcd->pdev->dev.parent, "Out-of-sync continued read\n");
			//break;
		case STATUS_IDLE:
		case STATUS_BUSY:
			...
			goto exit;
		default:
			LOGE(tcm_hcd->pdev->dev.parent, "Incorrect Status code (0x%02x)\n",
					tcm_hcd->status_report_code);
...
static int ovt_tcm_enable_irq(struct ovt_tcm_hcd *tcm_hcd, bool en, bool ns)
{
	...
	if (en) {
		...
		if (irq_freed) {
			// FIXME: possible deadlock even with IRQF_ONESHOT?!
			retval = request_threaded_irq(tcm_hcd->irq, NULL,
					ovt_tcm_isr, bdata->irq_flags | IRQF_ONESHOT,
...
/*prize add by liaoxingen 202030325 start */
#ifdef CONFIG_PRIZE_HARDWARE_INFO
	sprintf(current_tp_info.id,"0x%02x", tcm_hcd->packrat_number);
	strcpy(current_tp_info.chip, "td4321 fw:001");
	strcpy(current_tp_info.vendor, "omnivision_tcm");
	sprintf(current_tp_info.more, "%d*%d", 1080, 2340);  //prize modify by wangfei for TP info 20210810
#endif
/*prize add by liaoxingen 202030325 end */
exit:
		...
		bool hw, bool update_wd)
{
	...
	if (hw) {
		...
		LOGN(tcm_hcd->pdev->dev.parent,"wolf hard reset,reset_on_state = %d\n",bdata->reset_on_state);
		gpio_set_value(bdata->reset_gpio, bdata->reset_on_state);
		msleep(bdata->reset_active_ms);
		gpio_set_value(bdata->reset_gpio, !bdata->reset_on_state);
	} else {
		...
		LOGN(tcm_hcd->pdev->dev.parent,"wolf soft reset\n");
	}
		...
		unsigned long action, void *data)
{
	...
	if (evdata && evdata->data && tcm_hcd) {
		...
		// FIXME: FB_EARLY_EVENT_BLANK -> FB_EVENT_BLANK
		...
		if (action == FB_EVENT_BLANK) {
			if (*transition == FB_BLANK_POWERDOWN) {
				...
				tcm_hcd->fb_ready = 0;
				printk("wolf 000 fb_ready = %d\n",tcm_hcd->fb_ready);
			} else if (*transition == FB_BLANK_UNBLANK) {
				retval = ovt_tcm_resume(&tcm_hcd->pdev->dev);
				tcm_hcd->fb_ready++;
				printk("wolf 111 fb_ready = %d\n",tcm_hcd->fb_ready);
			}
		}
	}
...
f35_boot_recheck:
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to read F35 function number\n");
		tcm_hcd->is_detected = false;
		return -ENODEV;
	}

	LOGE(tcm_hcd->pdev->dev.parent, "Found F$%02x\n", fn_number);

	if (fn_number != RMI_UBL_FN_NUMBER) {
			LOGE(tcm_hcd->pdev->dev.parent, "Failed to find F$35, try_times = %d\n",
					retry);
		if (retry < retry_max) {
			msleep(100);
			gpio_set_value(bdata->reset_gpio, 0);
